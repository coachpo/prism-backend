...
def _convert_per_1k_price_to_per_1m(price: str | None) -> str | None:
    if price is None:
        return None
    value = price.strip()
    if not value:
        return price
    try:
        scaled = Decimal(value) * Decimal("1000")
    except InvalidOperation:
        return price

    scaled_str = format(scaled, "f")
    if "." in scaled_str:
        scaled_str = scaled_str.rstrip("0").rstrip(".")
    return scaled_str or "0"


async def _drop_connections_pricing_unit_if_needed(conn) -> None:
    if not await _table_exists(conn, "connections"):
        return

    connection_columns = await _table_columns(conn, "connections")
    if "pricing_unit" not in connection_columns:
        return
    ...
    missing_columns = required_columns - connection_columns
    if missing_columns:
        logger.warning(
            "Skipping connections.pricing_unit drop migration; missing columns: %s",
            sorted(missing_columns),
        )
        return

    await conn.execute(text("DROP TABLE IF EXISTS connections_tmp_pricing_unit_drop"))
    await conn.execute(
        text(
            """
            CREATE TABLE connections_tmp_pricing_unit_drop (
                id INTEGER PRIMARY KEY,
                model_config_id INTEGER NOT NULL,
                endpoint_id INTEGER NOT NULL,
                is_active BOOLEAN NOT NULL DEFAULT 1,
        ...
        await conn.execute(
            text(
                ...
                SELECT
                    id,
                    model_config_id,
                    COALESCE(endpoint_id, id) AS endpoint_id,
                    ...
                    cache_creation_price,
                    reasoning_price,
                    COALESCE(missing_special_token_price_policy, 'MAP_TO_OUTPUT') AS missing_special_token_price_policy,
                    COALESCE(pricing_config_version, 0) AS pricing_config_version,
                    COALESCE(forward_stream_options, 0) AS forward_stream_options,
                    created_at,
                    updated_at
                FROM connections
                """
    ...
    for row in rows:
        ...
        if pricing_unit == "PER_1K":
            converted_rows += 1
            input_price = _convert_per_1k_price_to_per_1m(input_price)
            output_price = _convert_per_1k_price_to_per_1m(output_price)
            cached_input_price = _convert_per_1k_price_to_per_1m(cached_input_price)
            cache_creation_price = _convert_per_1k_price_to_per_1m(cache_creation_price)
            reasoning_price = _convert_per_1k_price_to_per_1m(reasoning_price)
        ...
        records.append(
            {
                ...
                "missing_special_token_price_policy": row[
                    "missing_special_token_price_policy"
                ],
                "pricing_config_version": row["pricing_config_version"],
                "forward_stream_options": row["forward_stream_options"],
    ...
    if records:
        await conn.execute(
            text(
                """
                INSERT INTO connections_tmp_pricing_unit_drop (
                    id,
                    model_config_id,
                    endpoint_id,
                    is_active,
    ...
    await conn.execute(
        text(
            "ALTER TABLE connections_tmp_pricing_unit_drop RENAME TO connections"
        )
    )
    await conn.execute(
        text(
    ...
    await conn.execute(
        text("CREATE INDEX IF NOT EXISTS idx_connections_priority ON connections(priority)")
    )
    logger.info(
        "Migrated: dropped pricing_unit column from connections table (converted %d PER_1K rows)",
        converted_rows,
    )
