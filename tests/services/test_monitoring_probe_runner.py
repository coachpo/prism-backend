...
class TestMonitoringProbeRunner:
    ...
    @pytest.mark.asyncio
    async def test_run_connection_probe_preserves_original_failure_when_recovery_cleanup_commit_fails(
        ...
        def fake_session_factory() -> _RecoverySession:
            session = _RecoverySession(
                commit_error=RuntimeError("cleanup commit failed")
            )
            recovery_sessions.append(session)
            return session
        ...
        async def release_probe_lease_fn(
            *, session, profile_id: int, lease_token: str, now_at
            ...
            assert session is recovery_sessions[0]
            assert profile_id == 7
            assert lease_token == "lease-token"
            assert now_at is not None
            return True
        ...
        assert len(recovery_sessions) == 1
        assert recovery_sessions[0].commit_calls == 1
        assert recovery_sessions[0].committed is False
        assert "Monitoring probe lease cleanup failed" in caplog.text
