...
├── test_smoke_defect_regressions.py      # Top-level DEF aggregator, currently through DEF080
...
- `test_smoke_defect_regressions.py` is the top-level DEF aggregator and now exports cases through `TestDEF080_VendorApiFamilySchemaSplit`.
...
- Do not invent extra hierarchy docs under the existing smoke or isolation subtrees when the parent coverage already explains the shape. `smoke_defect_regressions/test_proxy_cases/AGENTS.md` is the current exception.
